...
void MainWindow::on_treeWidget_Plugins_itemDoubleClicked(QTreeWidgetItem *item, int column) {
    ...
    // If it's a parent, toggle its state
    // OPTION 1: Collapsed to expanded
    // OPTION 2: Expanded to collapsed
    // (Check is only valid for a two-level QTreeWidget (items with children are parents, items without children are children.)
    if (item->childCount() > 0) {

        if (item->isExpanded()) ui->treeWidget_Plugins->collapseItem(item);
        else ui->treeWidget_Plugins->expandItem(item);
    ...
    // If it's a child, perform the double-click action if it makes sense
        ...
        if (ui->treeWidget_Plugins->indexOfTopLevelItem(item->parent()) == 0) {
            ...
            // Hide QTreeWidgetItems that do not match the selected item
            for(int i = 0; i < ui->treeWidgetDataDefnitions->topLevelItemCount(); ++i) {

                QTreeWidgetItem *item2 = ui->treeWidgetDataDefnitions->topLevelItem(i);

                // Get items from column 1 (Type)
                QString type = item2->text(1);

                if (type != item->text(0)) item2->setHidden(true);
                else item2->setHidden(false);
